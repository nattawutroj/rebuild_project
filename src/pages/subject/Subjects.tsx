...
const Example = () => {
    ...
    const columns = useMemo(
        () => [
            ...
            {
                accessorKey: 'subjects_courses_id',
                header: 'รหัสหลักสูตร',
                size: 100,
                muiEditTextFieldProps: {
                    type: 'subjects_courses_id',
                    required: true,
                },
            },
        ],
    ...
    const handleCreateUser = async ({ values, table }: { values: any, table: any }) => {
        console.log(values);
        if (values.subjects_name !== '' && values.credit !== '') {
            Axios.post('/subjects/add', values)
                .then(res => {
    ...
    const handleSaveUser = async ({ values, table }: { values: any, table: any }) => {
        console.log(values);
        Axios.put('/subjects/edit', values)
            .then(res => {
                console.log(res.data);
                Axios.get('/subjects')
                    .then(res => {
    ...
    const openDeleteConfirmModal = (row:any) => {
        ...
        if (window.confirm(`คุณต้องการลบข้อมูลนี้หรือไม่?`)) {
            Axios.delete('/subjects/delete', { data: { subjects_code: row.original.subjects_code } })
                .then(res => {
                    console.log(res.data);
                    Axios.get('/subjects')
                        .then(res => {
    ...
    const table = useMaterialReactTable({
        ...
        initialState: {
            ...
            sorting: [{ id: 'subjects_code', desc: true }],
        },
        state: {
