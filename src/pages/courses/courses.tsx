...
const Example = () => {
    ...
    const handleCreateUser = async ({ values, table }: { values: any, table: any }) => {
        console.log(values);
        if (values.courses_code !== '' && values.courses_name !== '') {
            Axios.post('/course/add', values)
                .then(res => {
    ...
    const openDeleteConfirmModal = (row:any) => {
        ...
        if (window.confirm(`คุณต้องการลบข้อมูลนี้หรือไม่?`)) {
            Axios.delete('/course/delete', { data: { courses_id: row.original.courses_id } })
                .then(res => {
                    console.log(res.data);
                    Axios.get('/course')
                        .then(res => {
    ...
    const table = useMaterialReactTable({
        ...
        initialState: {
            ...
            sorting: [{ id: 'courses_id', desc: true }],
        },
        state: {
